...
export default function OrchestratorPlan({ plan, lastUpdated }: Props) {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="bg-stone-800/60 border border-stone-700/60 rounded-xl overflow-hidden">
        {headings.length > 0 ? (
          <div>
            {sections.map((section, i) => {
              ...
              return (
                <div key={i} className="border-b border-stone-800/50 last:border-0">
                  <div
                    className={`px-5 py-3 font-semibold ${
                      level === 1
                        ? 'text-lg text-stone-100 bg-stone-800/80'
                        : level === 2
                        ? 'text-base text-stone-200 bg-stone-800/40'
                  ...
                  {content && (
                    <pre className="px-5 py-3 text-sm text-stone-400 whitespace-pre-wrap font-sans leading-relaxed">
                      {content}
                    </pre>
                  )}
                </div>
          ...
          <pre className="p-5 text-sm text-stone-400 whitespace-pre-wrap font-mono leading-relaxed overflow-x-auto">
            {plan}
          </pre>
        )}
      </div>
